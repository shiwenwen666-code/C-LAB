...
const NO_MATERIAL_LABEL = "不指定材质";

const abstractMaterialTerms = [
  "材质关系",
  "表面质感",
  "细腻纹理",
  "光影层次",
...
const englishMaterialTerms = {
  [NO_MATERIAL_LABEL]: "unspecified material",
  "材质关系": "material relationships",
  "表面质感": "surface tactility",
  "细腻纹理": "fine texture",
  "光影层次": "layered lighting",
  "抽象肌理": "abstract material grain",
  "质感对比": "texture contrast",
  "柔和表面质感": "soft surface quality",
  "温润纹理关系": "warm subtle texture relationships",
  "工业质感表面": "industrial surface quality",
  "柔软亲肤触感": "soft skin-friendly tactile mood",
  "哑光与高光层次": "matte and glossy layering",
  "细腻反光关系": "subtle reflective relationships",
  "结构化表面关系": "structured surface relationships",
  "冷灰色材质层次": "cool gray material layering",
  "泛化材质表达": "generalized material expression"
};
...
let currentPrompts = [];
let lastSelectedMaterialValues = [];
...
function normalizeMaterialList(list) {
  return uniqueList(list)
    .filter((material) => material !== NO_MATERIAL_VALUE && material !== NO_MATERIAL_LABEL);
}

function buildMaterialOptions(materials) {
  return [
    { value: NO_MATERIAL_VALUE, label: NO_MATERIAL_LABEL },
    ...normalizeMaterialList(materials)
  ];
}
...
function bindEvents() {
  ...
  elements.tertiaryCategory.addEventListener("change", () => {
  ...
  elements.productConcreteness.addEventListener("change", updateMaterialOptions);
  elements.materials.addEventListener("change", handleMaterialSelectionChange);
...
function updateMaterialOptions() {
  const options = getMaterialOptionsForCurrentSelection();
  fillSelect(elements.materials, options);

  const shouldDefaultToNoMaterial = (
    isGeneralCategory(elements.tertiaryCategory.value) ||
    elements.generationScope.value === "通用行业氛围" ||
    isPureAtmosphere(elements.productConcreteness.value) ||
    isDistantWeak(elements.productConcreteness.value) ||
    isAbstractMode(elements.tertiaryCategory.value, elements.generationScope.value, elements.productConcreteness.value)
  );

  if (shouldDefaultToNoMaterial) {
    selectOnlyNoMaterial();
  }

  lastSelectedMaterialValues = getSelectedMaterials();
}

function getMaterialOptionsForCurrentSelection() {
  ...
  const categoryMaterials = (window.categoryMaterialData || {})[category]?.materials || [];

  if (isGeneralCategory(category) || isPureAtmosphere(concreteness) || isDistantWeak(concreteness) || isAbstractMode(category, scope, concreteness) || scope === "通用行业氛围") {
    return buildMaterialOptions([
      ...abstractMaterialTerms,
      ...(data.abstractCues || []).slice(0, 3),
      "泛化材质表达"
    ]);
  }
...
function selectOnlyNoMaterial() {
  Array.from(elements.materials.options).forEach((option) => {
    option.selected = option.value === NO_MATERIAL_VALUE;
  });
}

function handleMaterialSelectionChange() {
  const values = getSelectedMaterials();
  const hasNoMaterial = values.includes(NO_MATERIAL_VALUE);
  const concreteValues = values.filter((value) => value !== NO_MATERIAL_VALUE);
  const hadNoMaterial = lastSelectedMaterialValues.includes(NO_MATERIAL_VALUE);

  if (hasNoMaterial && concreteValues.length) {
    if (!hadNoMaterial) {
      selectOnlyNoMaterial();
    } else {
      Array.from(elements.materials.options).forEach((option) => {
        if (option.value === NO_MATERIAL_VALUE) option.selected = false;
      });
    }
  }
...
function hasNoMaterialSelection(materials) {
  return materials.includes(NO_MATERIAL_VALUE) || materials.includes(NO_MATERIAL_LABEL);
}

function shouldUseAbstractMaterials(category, scope, concreteness, materials) {
  return hasNoMaterialSelection(materials) ||
    isPureAtmosphere(concreteness) ||
    isDistantWeak(concreteness) ||
    isAbstractMode(category, scope, concreteness) ||
    scope === "通用行业氛围" ||
    (isGeneralCategory(category) && isGenericMode(category, scope));
}

function canUseConcreteMaterials(category, concreteness, materials) {
  return !hasNoMaterialSelection(materials) &&
    !isGeneralCategory(category) &&
    (isHintMode(concreteness) || concreteness === "明确产品") &&
    materials.length > 0;
}
...
function enMaterial(term) {
  return englishMaterialTerms[term] || en(term);
}
...
function getPromptMaterials(sceneData, category, scope, concreteness, selectedMaterials, seed) {
  if (shouldUseAbstractMaterials(category, scope, concreteness, selectedMaterials)) {
    return pickMany(abstractMaterialTerms, 4, seed);
  }

  if (canUseConcreteMaterials(category, concreteness, selectedMaterials)) {
    return selectedMaterials.filter((material) => material !== NO_MATERIAL_VALUE && material !== NO_MATERIAL_LABEL);
  }

  return pickMany(abstractMaterialTerms, 4, seed);
}

function getMaterialLineText({ lighting, selectedMaterials, pureMode, distantMode, abstractMode, genericAtmosphereMode, hintMode, genericMode, useAbstractMaterials }) {
  const zhMaterials = selectedMaterials.join("、");
  const enMaterials = selectedMaterials.map(enMaterial).join(", ");
  const zhAbstractMaterialExpression = "通过材质关系、表面质感、抽象肌理、光影层次和质感对比来表达行业气质";
  const enAbstractMaterialExpression = "Convey the industry through material relationships, surface texture, abstract tactile qualities, lighting layers, and subtle texture contrast.";

  if (useAbstractMaterials) {
    if (pureMode) {
      return {
        zh: `采用${lighting}，${zhAbstractMaterialExpression}，同时组织空间、色彩、光影和抽象陈列关系，不写具体材质名，不让材质指向明确商品或可售卖产品。`,
        en: `Use ${en(lighting)}. ${enAbstractMaterialExpression} Organize space, color, lighting, and abstract display relationships without naming specific materials or implying a clear sellable product.`
      };
    }

    if (distantMode) {
      return {
        zh: `采用${lighting}，${zhAbstractMaterialExpression}；材质只作为远景气质、背景轮廓和表面层次的弱提示，不精确到具体产品材质，也不形成清晰商品识别点。`,
        en: `Use ${en(lighting)}. ${enAbstractMaterialExpression} Material cues should remain only weak references in distant depth, background outlines, and surface layering, without specifying product materials or creating a clear product identifier.`
      };
    }

    if (abstractMode || genericAtmosphereMode) {
      return {
        zh: `采用${lighting}，${zhAbstractMaterialExpression}，结合色彩层次、空间留白和光影氛围建立品牌背景感；材质只作为抽象质感线索，不成为具体商品识别点。`,
        en: `Use ${en(lighting)}. ${enAbstractMaterialExpression} Combine this with layered color, spatial negative space, and lighting atmosphere to create a brand-background feeling. Materials should remain abstract texture cues, not concrete product identifiers.`
      };
    }

    return {
      zh: `采用${lighting}，${zhAbstractMaterialExpression}；让环境层次、辅助元素、表面质感和阴影关系清晰可控，材质表达保持泛化，不写具体材质名。`,
      en: `Use ${en(lighting)}. ${enAbstractMaterialExpression} Shape environmental depth, supporting elements, surface tactility, and shadow relationships while keeping material expression generalized.`
    };
  }

  if (hintMode || genericMode) {
    return {
      zh: `采用${lighting}，让环境层次、辅助元素、材质高光和阴影关系清晰可控；可轻度表现${zhMaterials}等材质纹理，但产品元素只作为氛围线索。`,
      en: `Use ${en(lighting)} to shape environmental depth, supporting elements, material highlights, and shadow relationships. Lightly show material textures such as ${enMaterials}, while keeping product elements as atmospheric cues only.`
    };
  }

  return {
    zh: `采用${lighting}，让主体轮廓、表面高光和阴影层次清晰可控；可明确表现${zhMaterials}等材质纹理，细节真实，触感明确。`,
    en: `Use ${en(lighting)} to create controlled highlights, readable shadows, and a clear subject silhouette. Clearly show material details such as ${enMaterials}, with realistic tactile texture.`
  };
}
...
function buildPrompt(index) {
  ...
  const selectedMaterials = getPromptMaterials(sceneData, category, scope, concreteness, materials, expressionSeed + index);
  const useAbstractMaterials = shouldUseAbstractMaterials(category, scope, concreteness, materials) || !canUseConcreteMaterials(category, concreteness, materials);
  ...
  const materialLine = getMaterialLineText({
    lighting,
    selectedMaterials,
    pureMode,
    distantMode,
    ...
    hintMode,
    genericMode,
    useAbstractMaterials
  });
  const zhMaterialLine = materialLine.zh;
  ...
  const enMaterialLine = materialLine.en;
